#define bkgtree_analyser_cxx
#include "bkgtree_analyser.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void bkgtree_analyser::Loop()
{
//   In a ROOT session, you can do:
//      root> .L bkgtree_analyser.C
//      root> bkgtree_analyser t
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	matchedphoton = 0;
	for(unsigned k =0; k<nMC;k++){
	  if((*mcStatus)[k]==1 && abs((*mcPID)[k])==22){
	    
	    float dp = std::abs((*mcPhi)[k] - (*elePhi)[j]);
	    if(dp > (float)(M_PI)) dp -= (float)(2 * M_PI);
	    float de = (*eleEta)[j]-(*mcEta)[k];
	    float dr = std::sqrt(dp*dp+de*de);
	    //if((std::sqrt(dp*dp+((*eleEta)[j]-(*mcEta)[k])*((*eleEta)[j]-(*mcEta)[k]))<0.04) && (((*elePt)[j]/(*mcPt)[k])>0.7) &&   (((*elePt)[j]/(*mcPt)[k])<1.3)) matchedphoton = 1;
	    if((dr<0.04) && (((*elePt)[j]/(*mcPt)[k])>0.7) && (((*elePt)[j]/(*mcPt)[k])<1.3)) matchedphoton = 1;
	    if(matchedphoton){
	      std::cout<<"DPhi is "<<dp<<" for electron phi = "<<(*elePhi)[j]<<" and MC Phi = "<<(*mcPhi)[k]<<std::endl;
	      std::cout<<"DEta is "<<de<<" for electron eta = "<<(*eleEta)[j]<<" and MC Eta = "<<(*mcEta)[k]<<std::endl;
	      std::cout<<"DR is "<<dr<<std::endl;	    
	      std::cout<<"The electron is matched to photon? "<<matchedphoton<<std::endl;
	      break;
	    }
	  }
	}
	if((*elePt)[j]<5 || (*ele_genmatched)[j]==1 || matchedphoton ==1) continue;
	...
	//std::cout<<"In this event myR91 and myR92 = "<<myR9_1<<", "<<myR9_2<<std::endl;
	//std::cout<<"In this event genweight = "<<genWeight<<std::endl;
	for(int i=0;i<sizeof(hist_min)/sizeof(hist_min[0]);i++){
	  if(jentry%10000 == 0) std::cout<<"In "<<jentry<<"th entry, " << hname[i]<<" histogram is being filled"<<std::endl;
	  if(i==0 && std::abs((*eleSCEta)[j])<1.479){
	    if((*eleR9Full5x5)[j]>=-1.0) hist[i]->Fill((*eleR9Full5x5)[j],genWeight);
	  }
	  if(i==1 && std::abs((*eleSCEta)[j])>1.479 && std::abs((*eleSCEta)[j])<2.5){
	    if((*eleR9Full5x5)[j]>=-1.0) hist[i]->Fill((*eleR9Full5x5)[j],genWeight);
	  }
	  if(i==2+0 && std::abs((*eleSCEta)[j])<1.479){
	    if((*eleR9Full5x5)[j]>=-1.0) hist[i]->Fill(myR9_1,genWeight);
	  }
	  if(i==2+1 && std::abs((*eleSCEta)[j])>1.479 && std::abs((*eleSCEta)[j])<2.5){
	    if((*eleR9Full5x5)[j]>=-1.0) hist[i]->Fill(myR9_1,genWeight);
	  }
	  if(i==4+0 && std::abs((*eleSCEta)[j])<1.479){
	    if((*eleR9Full5x5)[j]>=-1.0) hist[i]->Fill(myR9_2,genWeight);
	  }
	  if(i==4+1 && std::abs((*eleSCEta)[j])>1.479 && std::abs((*eleSCEta)[j])<2.5){
	    if((*eleR9Full5x5)[j]>=-1.0) hist[i]->Fill(myR9_2,genWeight);
	  }
	  if(i==6+0 && std::abs((*eleSCEta)[j])<1.479){
	    if((*eleR9Full5x5)[j]>=-1.0) hist[i]->Fill(cmsR9_2,genWeight);
	  }
	  if(i==6+1 && std::abs((*eleSCEta)[j])>1.479 && std::abs((*eleSCEta)[j])<2.5){
	    if((*eleR9Full5x5)[j]>=-1.0) hist[i]->Fill(cmsR9_2,genWeight);
	  }
	...
	for(int i=0;i<sizeof(histrange_min)/sizeof(histrange_min[0]);i++){
	  ...
	  if(i==8+0 && std::abs((*eleSCEta)[j])<1.479){
	    if(myR9_1>=-1.0) hprof[i]->Fill((*elePt)[j],myR9_1,genWeight);
	  }
	  if(i==8+1 && std::abs((*eleSCEta)[j])>1.479 && std::abs((*eleSCEta)[j])<2.5){
	    if(myR9_1>=-1.0) hprof[i]->Fill((*elePt)[j],myR9_1,genWeight);
	  }
	  if(i==8+2 && std::abs((*eleSCEta)[j])<2.5){
	    if(myR9_1>=-1.0) hprof[i]->Fill((*eleSCEta)[j],myR9_1,genWeight);
	  }
	  if(i==8+3 && std::abs((*eleSCEta)[j])<2.5){
	    if(myR9_1>=-1.0) hprof[i]->Fill((*eleSCPhi)[j],myR9_1,genWeight);
	  }
	  if(i==8+4 && std::abs((*eleSCEta)[j])<1.479){
	    if(myR9_1>=-1.0) hprof[i]->Fill(nVtx,myR9_1,genWeight);
	  }
	  if(i==8+5 && std::abs((*eleSCEta)[j])>1.479 && std::abs((*eleSCEta)[j])<2.5){
	    if(myR9_1>=-1.0) hprof[i]->Fill(nVtx,myR9_1,genWeight);
	  }
	  if(i==8+6 && std::abs((*eleSCEta)[j])<1.479){
	    if(myR9_1>=-1.0) hprof[i]->Fill(rho,myR9_1,genWeight);
	  }
	  if(i==8+7 && std::abs((*eleSCEta)[j])>1.479 && std::abs((*eleSCEta)[j])<2.5){
	    if(myR9_1>=-1.0) hprof[i]->Fill(rho,myR9_1,genWeight);
	  }
	  ...
	  if(i==16+0 && std::abs((*eleSCEta)[j])<1.479){
	    if(myR9_2>=-1.0) hprof[i]->Fill((*elePt)[j],myR9_2,genWeight);
	  }
	  if(i==16+1 && std::abs((*eleSCEta)[j])>1.479 && std::abs((*eleSCEta)[j])<2.5){
	    if(myR9_2>=-1.0) hprof[i]->Fill((*elePt)[j],myR9_2,genWeight);
	  }
	  if(i==16+2 && std::abs((*eleSCEta)[j])<2.5){
	    if(myR9_2>=-1.0) hprof[i]->Fill((*eleSCEta)[j],myR9_2,genWeight);
	  }
	  if(i==16+3 && std::abs((*eleSCEta)[j])<2.5){
	    if(myR9_2>=-1.0) hprof[i]->Fill((*eleSCPhi)[j],myR9_2,genWeight);
	  }
	  if(i==16+4 && std::abs((*eleSCEta)[j])<1.479){
	    if(myR9_2>=-1.0) hprof[i]->Fill(nVtx,myR9_2,genWeight);
	  }
	  if(i==16+5 && std::abs((*eleSCEta)[j])>1.479 && std::abs((*eleSCEta)[j])<2.5){
	    if(myR9_2>=-1.0) hprof[i]->Fill(nVtx,myR9_2,genWeight);
	  }
	  if(i==16+6 && std::abs((*eleSCEta)[j])<1.479){
	    if(myR9_2>=-1.0) hprof[i]->Fill(rho,myR9_2,genWeight);
	  }
	  if(i==16+7 && std::abs((*eleSCEta)[j])>1.479 && std::abs((*eleSCEta)[j])<2.5){
	    if(myR9_2>=-1.0) hprof[i]->Fill(rho,myR9_2,genWeight);
	  }
   ...
   TFile *outputfile = new TFile("Outputfile_histograms_QCDMC2024_30Jan.root","RECREATE");
